...
export async function GET(request: NextRequest) {
  try {
    ...
    logger.error('Failed to fetch quests', { error: String(error) });
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    logger.error('Failed to process quest action', { error: String(error) });
    return NextResponse.json(
      { success: false, error: 'Failed to process quest action' },
      { status: 500 }
    );
  }
}
