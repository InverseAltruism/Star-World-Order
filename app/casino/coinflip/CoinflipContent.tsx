// CoinflipContent — wagmi-wired wrapper around `<CoinflipPanel />` for
// /casino/coinflip. All side-effecting state (wallet, chain, writeContract,
// BetSettled subscription) lives here so the presentational panel stays
// trivially testable.
//
// Lifecycle:
//   1. Resolve `CosmicFlip` address for the active chain.
//   2. If wagmi reports a chain outside {143, 10143}, surface a "Switch to
//      Monad" CTA via `useSwitchChain`.
//   3. `placeBet(sideEnum, clientSeed, serverCommit)` is broadcast through
//      `useWriteContract`. `clientSeed` is generated locally; `serverCommit`
//      is read from `NEXT_PUBLIC_COSMIC_FLIP_COMMIT` (or a placeholder zero
